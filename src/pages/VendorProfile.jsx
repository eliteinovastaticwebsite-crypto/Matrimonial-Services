            ...
            <Field label="Type of Service">
              <div className="flex flex-wrap gap-1.5 mt-1">
                {vendorData.basicDetails.typeOfService.map((service, i) => (
                  <span key={i} className="inline-flex items-center bg-red-50 text-red-700 text-xs px-2 py-0.5 rounded-full">
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Field label="Current Location">
          <Val>{vendorData.contactInfo.currentLocation}</Val>
        </Field>
        <Field label="Website">
            ...
            <Field label="Camera Models Used">
              <div className="flex flex-wrap gap-1.5 mt-1">
                {vendorData.equipmentTeam.cameraModels.map((c, i) => (
                  <span key={i} className="inline-flex items-center bg-yellow-50 text-yellow-700 text-xs px-2 py-0.5 rounded-full">{c}</span>
                ))}
              </div>
            </Field>
            <Field label="Video Equipment / Drone">
              <div className="flex flex-wrap gap-1.5 mt-1">
                {vendorData.equipmentTeam.videoEquipment.map((e, i) => (
                  <span key={i} className="inline-flex items-center bg-blue-50 text-blue-700 text-xs px-2 py-0.5 rounded-full">{e}</span>
                ))}
              </div>
            </Field>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <Field label="Preferred Wedding Locations">
              <div className="flex flex-wrap gap-1.5 mt-1">
                {vendorData.serviceCoverage.preferredLocations.map((loc, i) => (
                  <span key={i} className="inline-flex items-center bg-green-50 text-green-700 text-xs px-2 py-0.5 rounded-full">
      ...
      case 'pricing':
        return (
          <div className="space-y-3">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="bg-gradient-to-br from-red-50 to-yellow-50 p-3 rounded-lg border border-red-100">
                <h4 className="text-xs font-bold text-gray-600 mb-1">Basic Wedding Package</h4>
                <p className="text-lg md:text-xl font-bold text-red-700">{vendorData.packagesPricing.basicWeddingPackage}</p>
              </div>
              <div className="bg-gradient-to-br from-yellow-50 to-red-50 p-3 rounded-lg border border-yellow-100">
                <h4 className="text-xs font-bold text-gray-600 mb-1">Full Wedding Package</h4>
                <p className="text-lg md:text-xl font-bold text-red-700">{vendorData.packagesPricing.fullWeddingPackage}</p>
              </div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              ...
              <div key={i} className="bg-gradient-to-br from-red-50 to-yellow-50 p-4 rounded-lg border border-red-100 text-center">
                <h4 className="text-xs font-bold text-gray-600 mb-2">{item.label}</h4>
                <p className="text-xl md:text-2xl font-bold text-red-700">{item.val}</p>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {vendorData.workPhotos.map((photo, i) => (
                  <div key={i} className="rounded-lg overflow-hidden border border-gray-200 bg-gray-100 h-20 flex items-center justify-center">
                    {photo ? (
    ...
    <div className="min-h-screen bg-gradient-to-b from-red-50 to-yellow-50">
      <div className="container mx-auto px-3 py-3 md:px-6 md:py-6">
        ...
        <nav className="mb-3 md:mb-6">
          <ol className="flex items-center space-x-1 text-xs md:text-sm">
        ...
        <div className="bg-white rounded-lg shadow-lg border border-red-200 overflow-hidden">
          ...
          <div className="relative bg-gradient-to-r from-red-600 to-red-700 p-3 md:p-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-6">
              {/* Logo */}
              <div className="w-16 h-16 sm:w-24 sm:h-24 rounded-full border-4 border-white shadow-lg overflow-hidden bg-gradient-to-br from-red-50 to-yellow-50 flex-shrink-0">
                {vendorData.basicDetails.logo ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-red-100">
                    <svg className="w-10 h-10 text-red-400 md:w-12 md:h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className="inline-flex items-center gap-1.5 mt-1 bg-yellow-400 text-red-800 px-2 py-0.5 rounded-full text-xs md:text-sm font-bold shadow-sm">
                ...
                <div className="text-white/90 text-xs sm:text-sm md:text-base space-y-1 mt-2 md:mt-3">
                ...
                <div className="flex flex-wrap gap-2 mt-2 md:mt-3">
                  {[
                    ...
                    <button key={i} className="bg-white text-red-700 hover:bg-red-50 px-2 py-1 rounded text-xs md:text-sm font-bold flex items-center gap-1 md:px-3 md:py-1.5">
                      {btn.icon === 'phone' && (
                        <svg className="w-3 h-3 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="border-t border-gray-200">
                ...
                <div className="flex w-full min-w-max border-b border-gray-200">
                  {sections.map((section) => (
                    <button
                      ...
                      className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-2 md:px-4 md:py-3 border-r border-gray-200 last:border-r-0 transition-all ${
              ...
              <div className="p-3 md:p-6">
                <div className="flex items-center mb-3 md:mb-4">
                  ...
                  <div className="ml-2 w-8 md:w-12 h-0.5 bg-gradient-to-r from-red-600 to-yellow-500 rounded-full"></div>
                </div>
                <div className="bg-gradient-to-br from-red-50 to-yellow-50 p-3 md:p-6 rounded-lg border border-red-100">
                  ...
                  <div className="mt-4 pt-3 md:pt-4 border-t border-red-200 flex flex-col sm:flex-row justify-between items-center gap-3">
                    ...
                    {/* View Full Details Button - Shorter version within the section */}
                    <button
                      onClick={() => setShowFullDetails(true)}
                      className="bg-gradient-to-r from-yellow-500 to-yellow-600 text-red-900 py-2 px-4 rounded-lg font-bold hover:from-yellow-600 hover:to-yellow-700 transition-all shadow flex items-center justify-center gap-2 text-sm"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="p-3 md:p-6 bg-gradient-to-r from-yellow-50 to-red-50 border-t border-gray-200">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-6">
                ...
                <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-yellow-200">
                  <h3 className="text-xs md:text-sm font-bold text-red-700 mb-2 flex items-center">
                    <svg className="w-4 h-4 md:w-5 md:h-5 mr-1.5 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                    </svg>
                    Primary Vendor Address
                  </h3>
                  <div className="space-y-1.5 md:space-y-2 text-xs md:text-sm">
                    ...
                    <div className="flex items-start gap-2">
                      <svg className="w-3.5 h-3.5 md:w-4 md:h-4 text-gray-500 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                      </svg>
                      <span className="font-medium text-gray-800">
                ...
                <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-red-200">
                  <h3 className="text-xs md:text-sm font-bold text-red-700 mb-2 flex items-center">
                    <svg className="w-4 h-4 md:w-5 md:h-5 mr-1.5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                    </svg>
                    Office Address
                  </h3>
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 md:gap-2 text-xs md:text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-gray-800">Mon-Sat: 9AM-7PM</span>
                    </div>
                    <div className="flex items-start gap-2 sm:col-span-2">
                      <svg className="w-3.5 h-3.5 md:w-4 md:h-4 text-red-600 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                      </svg>
                      <span className="font-medium text-gray-800">123 Elite Plaza, Wedding Street, Chennai - 600001</span>
