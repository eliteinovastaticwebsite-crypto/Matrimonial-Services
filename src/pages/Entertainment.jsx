...
const Entertainment = () => {
  ...
  // Extended filter states based on Entertainment Vendor Registration form
  ...
  const [ledEffects, setLedEffects] = useState('');
  const [powerRequirement, setPowerRequirement] = useState('');
  const [backupEquipment, setBackupEquipment] = useState('');
  const [teamSize, setTeamSize] = useState('');
  const [uniformProvided, setUniformProvided] = useState('');
  const [setupIncluded, setSetupIncluded] = useState('');
  ...
  // Filter options from Entertainment Vendor Registration form
  const serviceTypes = [
    "DJ & Music",
    "Live Band / Orchestra",
  ...
  const handleEventCoverageToggle = (event) => {
  ...
  const filteredVendors = vendors.filter(vendor => {
    ...
    // Event coverage filter
    if (selectedEvents.length > 0) {
      if (!vendor.eventCoverage) return false;
      const hasMatchingEvent = selectedEvents.some(event => 
        vendor.eventCoverage.includes(event)
      );
    ...
    // Performance duration filter
    if (performanceDuration) {
      const minDuration = parseInt(performanceDuration) || 0;
      if (vendor.performanceDuration < minDuration) return false;
    }
    
    // Custom performances filter
    if (customPerformances && vendor.customPerformances !== customPerformances) return false;
    
    // Equipment filters
    if (soundSystem && vendor.soundSystem !== soundSystem) return false;
    ...
    // Setup included filter
    if (setupIncluded && vendor.setupIncluded !== setupIncluded) return false;
    ...
    // Noise restrictions awareness filter
    if (noiseRestrictions && vendor.noiseRestrictions !== noiseRestrictions) return false;
  ...
  const getActiveFilterCount = () => {
    ...
    if (experience) count++;
    if (performanceDuration) count++;
    if (customPerformances) count++;
    if (soundSystem) count++;
    if (lightingSetup) count++;
    if (ledEffects) count++;
    if (powerRequirement) count++;
    if (backupEquipment) count++;
    if (teamSize) count++;
    if (uniformProvided) count++;
    if (setupIncluded) count++;
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1 pr-2">
                      <h3 className="text-xs font-bold text-white leading-tight" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                        {selectedEvent ? `${filteredVendors.length} ${selectedEvent} Vendors` : `${filteredVendors.length} Entertainment Vendors`}
                      </h3>
                    ...
                    <div className="flex flex-col items-end">
                      <div className="bg-white/20 px-1.5 py-0.5 rounded mb-0.5">
                        ...
                        <span className="text-yellow-300 ml-0.5 text-[8px]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>₹8,000 - ₹2,00,000</span>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex flex-col items-end">
                      <div className="bg-white/20 px-3 py-1.5 rounded">
                        ...
                        <span className="text-yellow-300 ml-1.5 font-bold text-base" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>₹8,000 - ₹2,00,000</span>
                      </div>
                    </div>
                  </div>
